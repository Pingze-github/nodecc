...
function getRandomChild(childs) {
  const randomIndex = parseInt(Math.random() * childs.length);
  const child = childs[randomIndex];
  return {
    pid: child.pid,
    child
  }
...
function forkRun(child, param) {
  return new Promise(resolve => {
    const tid = new Date().getTime().toString() + child.pid;
  ...
  run() {
    return new Promise(resolve => {
      const loop = () => {
        ...
        const {pid, child} = getRandomChild(this.childs);
...
function Nodecc(task, queue, cmax, pmax) {
  ...
  this.childs = __initChilds(pmax || (cmax < CPUNUM ? cmax : CPUNUM));
